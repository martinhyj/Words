...
class youdao():
    def __init__(self, type=0, word='hellow'):
        ...
        # 判断是否存在美音库
        if not os.path.exists(self._dirRoot+'/Speech_US'):
            # 不存在，就创建
            os.makedirs(self._dirRoot+'/Speech_US')
        # 判断是否存在英音库
        if not os.path.exists(self._dirRoot+'/Speech_EN'):
            # 不存在，就创建
            os.makedirs(self._dirRoot+'/Speech_EN')
    ...
    def down(self, word):
        ...
        if tmp is None:
            ...
            # 下载到目标地址
            try:
                request.urlretrieve(self._url, filename=self._filePath)
